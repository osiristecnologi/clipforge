...
(function() {
  ...
  function populateTemplates() {
    ...
    const templates = [
      ...
      { name: 'Unboxing Tech', cat: 'Shorts', color: '#3a2a1a', badge: 'Trending' },
    ];
    carousel.innerHTML = templates.map((t, i) => `
      <div class="template-card" onclick="navigateTo('editor')" style="animation-delay:${i*.05}s">
        ${t.badge ? `<span class="template-badge">${t.badge}</span>` : ''}
        <div class="template-thumb" style="background:linear-gradient(135deg, ${t.color}, ${t.color}dd);">
          <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;">
            <svg viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,.15)" stroke-width="1" style="width:64px;height:64px">
              <polygon points="5 3 19 12 5 21 5 3"/>
            </svg>
          </div>
          <div class="template-info">
            <h4>${t.name}</h4>
            <small>${t.cat} • 15s</small>
          </div>
        </div>
      </div>
    `).join('');
